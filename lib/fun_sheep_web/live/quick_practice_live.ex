defmodule FunSheepWeb.QuickPracticeLive do
  @moduledoc """
  Full-screen, swipeable "Tinder for learning" experience.

  Mobile/tablet only — on desktop (lg+) this redirects to /dashboard.
  Loads questions across ALL enrolled courses and presents them as
  ...
  def mount(_params, _session, socket) do
    ...
    state = QuickTestEngine.start_session(user_role_id, %{limit: @batch_size})
  ...
  def render(assigns) do
    ...
    <div class="lg:hidden fixed inset-0 z-30 bg-[#F5F5F7] flex flex-col" style="top: 0;">
      ...
      <%!-- Stats pills --%>
      <div :if={!@session_complete} class="flex items-center justify-center gap-3 px-4 py-2 shrink-0">
      ...
      <div class="flex-1 flex items-center justify-center px-4 py-4 overflow-hidden">
        ...
        >
          ...
          <div class="flex items-center justify-center gap-2 flex-wrap mb-4">
            ...
            >
              <.icon name="hero-question-mark-circle" class="w-3.5 h-3.5 text-[#FF3B30]" /> Why wrong?
            </button>
            <button
              phx-click="tutor_quick_action"
      ...
      >
        <div class="flex items-center justify-center gap-4 max-w-sm mx-auto">
          ...
          <button
            ...
            class="w-16 h-16 rounded-full bg-[#007AFF] active:bg-[#0066DD] flex items-center justify-center shadow-lg transition-colors touch-target"
            aria-label="Answer this question"
          >
            <.icon name="hero-pencil" class="w-8 h-8 text-white" />
          </button>
        ...
        <p :if={@question_number <= 2} class="text-center text-[10px] text-[#C7C7CC] mt-2">
          Swipe right = know it · left = learn · up = skip
        </p>
      </div>
    ...
    <%!-- Desktop fallback: redirect hint --%>
    <div class="hidden lg:block">
      <div class="max-w-lg mx-auto text-center py-16">
        <div class="text-6xl mb-4">📱</div>
        <h2 class="text-2xl font-bold text-gray-900 mb-3">Quick Practice</h2>
        <p class="text-[#8E8E93] mb-6">
          Quick Practice is optimized for mobile and tablet. Open this page on your phone for the full swipe experience!
        </p>
        <p class="text-sm text-[#C7C7CC] mb-6">
          Or use course-specific practice modes from your dashboard.
        </p>
        <.link
          navigate={~p"/dashboard"}
