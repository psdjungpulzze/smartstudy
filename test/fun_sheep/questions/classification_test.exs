defmodule FunSheep.Questions.ClassificationTest do
  @moduledoc """
  Tests for the classification foundation that backs North Star invariants
  I-1 (fine-grained skill tags) and I-15 (honest diagnostic filtering).
  """
  ...
  describe "tagged_for_adaptive/1" do
    test "includes only questions with section_id set AND trusted classification" do
  ...
  describe "classification_coverage/1" do
    test "counts tagged, untagged, low_confidence per course and per chapter" do
      %{course: course, chapter: chapter, section: section} = setup_course()
      ...
      assert chapter_row.total == 4
      assert chapter_row.tagged == 2
      assert chapter_row.untagged == 1
    end
  end
