defmodule FunSheep.StorageTest do
  ...
  describe "Storage behaviour delegation" do
    test "delegates to configured backend" do
      assert {:ok, @test_path} = FunSheep.Storage.put(@test_path, @test_content)
      assert {:ok, @test_content} = FunSheep.Storage.get(@test_path)
      assert "/uploads/#{@test_path}" == FunSheep.Storage.url(@test_path)
      assert :ok = FunSheep.Storage.delete(@test_path)
    end
  end
end
